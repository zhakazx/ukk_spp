<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 bg-slate-900 fixed-start " id="sidenav-main">
    <div class="sidenav-header">
      ...
      <a class="navbar-brand d-flex align-items-center m-0" href=" https://demos.creative-tim.com/corporate-ui-dashboard/pages/dashboard.html " target="_blank">
        <span class="font-weight-bold text-lg">UKK SPP</span>
      </a>
    </div>
    <div class="collapse navbar-collapse px-4 w-auto" id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        @if ($sidebar['level'] == null or in_array(auth()->user()->level, $sidebar['level']))
          @if ($sidebar['_type'] == 'dropdown')
            <li class="nav-item">
              <div class="nav-link {{ request()->routeIs($sidebar['exactUrl'].'*') ? 'active' : ''  }}" 
                  ...
                  aria-expanded="false" aria-controls="{{ $sidebar['menu'] }}">
                <div class="icon icon-shape icon-sm px-0 text-center d-flex align-items-center justify-content-center">
              ...
              <ul class="collapse" id="{{ $sidebar['menu'] }}">
                @foreach ($sidebar['submenu'] as $submenu)
                  <li class="nav-item border-start my-0 pt-2 list-unstyled">
                    <a class="nav-link position-relative ms-0 ps-2 py-2 " href="{{ route($submenu['route']) }}">
          ...
          @elseif($sidebar['_type'] == 'heading')
          <li class="nav-item">
            <a href="{{ route($sidebar['route']) }}" class="nav-link {{ request()->routeIs($sidebar['route']) == $sidebar['route'] ? 'active' : '' }}" aria-controls="{{ $sidebar['menu'] }}">
              <div class="icon icon-shape icon-sm px-0 text-center d-flex align-items-center justify-content-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" width="20px" height="20px">
                  {!! $sidebar['icon'] !!}
                </svg>
